...
final class GOLiveDataService: ObservableObject {
    ...
    private func fetchLiveFeed() async throws -> Data {
        ...
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            print("[GOLiveDataService] Metrolinx API returned error response. Falling back to local snapshot.")
            if let data = loadLocalSnapshot() {
                return data
            }
    ...
    func lookupTrainSchedule(trainNumber: String, departureDate: Date) async -> GOLiveTrain? {
        ...
        do {
            ...
            var fallbackTrain: GOLiveTrain? = nil
    ...
    func findTrains(originCode: String, destinationCode: String, date: Date) async -> [String: GOLiveTrain] {
        let rawData: Data?
        if useLocalSnapshot {
            rawData = loadLocalSnapshot()
        } else {
        ...
        do {
            ...
            var matches: [String: GOLiveTrain] = [:]
            ...
            print("[GOLiveDataService] findTrains failed: \(error)")
        }
        return [:]
    }
